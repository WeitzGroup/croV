...
switch type,
    case {'line'}
        ...
        line_width = (xdata(2)-xdata(1))*rescale/L_R;       % rescaled linewidth to match original
        spacer = xdata(1)*rescale/L_R;                      % rescaled spacer used for margins
    case {'hggroup'}
...
for i=1:numlines,
    ...
    switch type,
        case {'line'}
            ...
            set(object_h(linenum), 'xdata', [col/numcolumns/St_R+spacer col/numcolumns/St_R+spacer+line_width]);
...
switch lower(location),
    ...
    case {'northeast'}
        set(legend_h, 'position', [St_R2+pos(1)+fig_pos(3)-pos(3)-padding pos(2) pos(3) pos(4)]);
    case {'northwest'}
        set(legend_h, 'position', [pos(1)+padding pos(2) pos(3) pos(4)]);        
    case {'southeast'}
        pos(4) = newheight;
        set(legend_h, 'position', [St_R2+pos(1)+fig_pos(3)-pos(3)-padding fig_pos(2)-pos(4)/2+pos(4)/4+.02 pos(3) pos(4)]);
    case {'southwest'}
        set(legend_h, 'position', [fig_pos(1)+padding fig_pos(2)-pos(4)/2+pos(4)/4-.2 pos(3) pos(4)]);
